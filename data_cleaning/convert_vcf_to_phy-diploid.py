...
# Takes a vcf file from ipyrad and writes the data in phylip format. This version treats genotypes as diploid so it can handle diploid data or mixes of haploid and diploid data. Heterozygote genotypes will be coded as IUPAC ambiguity codes.
        ...
        if line[0:2] != "##": # skip the header lines
            ...
            if elements[0] == "#CHROM": # Line with the sample names
                ...
                for genotype_string in elements[9:]:
                    allele1 = genotype_string[0]
                    allele2 = genotype_string[2]
                    if allele1 == ".":
                        allele_character = "N" #Use N's for missing data
                    else:
                        allele_character = iupac_codes[alleles[int(allele1)] + alleles[int(allele2)]]
