...
# make_summary_for_block_sizes-min.py
...
# This version is based on minimum supported block sizes (the distance from the first introgressed allele to the last introgressed allele in a block, counting undetermined blocks as non-introgressed). This means the estimates of maximum block size are conservative estimates (i.e., the true maximum block size is probably not smaller than the estimate).

# usage: ./make_summary_for_block_sizes-min.py sample_species_table.csv > block_counts_and_sizes-min.csv
...
for chromosome_number in range(1,20):# The current chromosome
    ...
    with open(filename) as allind_file:
        if is_a_block: # we ended the last chromosome file in a block
            block_size = current_block_end + 1 - current_block_start
